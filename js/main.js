...
const X_MARKER_HTML = `
<svg class="gameMarker" style="enable-background:new 0 0 24 24;" version="1.1" viewBox="0 0 24 24" xml:space="preserve"
	...
	const markBoard = (to_mark, row_index, column_index) => {
		...
		if (
			...
			if (board[row_index][column_index] !== NOT_MARKED) {
				window.alert(
					"The spot has already been marked. Please choose another location."
				);
				return false;
			}
...
boardCells.forEach((cell) => {
	cell.addEventListener("click", (e) => {
		...
		if (gameBoard.markBoard(to_mark, row_index, column_index)) {
			updateDisplayBoard();
			if (checkGameOver() > 0) {
				return;
			} else {
				gameBoard.markBoard(computer.symbol, ...getComputerMove());
				updateDisplayBoard();
				checkGameOver();
			}
		}
...
const getComputerMove = () => {
	while (true) {
		let row_index = Math.floor(Math.random() * 3);
		let column_index = Math.floor(Math.random() * 3);
		if (gameBoard.getMarkAt(row_index, column_index) === NOT_MARKED) {
...
const updateDisplayBoard = () => {
	...
	boardCells.forEach((cell) => {
		...
		if (mark === X_MARKER) {
			cell.appendChild(createMarkerContainer(X_MARKER));
			console.log("Adding mark X");
		} else if (mark === O_MARKER) {
			cell.appendChild(createMarkerContainer(O_MARKER));
			console.log("Adding mark O at " + row_index + ", " + column_index);
		}
	});
};

const createMarkerContainer = (marker) => {
	let markerContainer = document.createElement("div");
	markerContainer.innerHTML =
		marker === X_MARKER ? X_MARKER_HTML : O_MARKER_HTML;
